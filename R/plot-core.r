...
.get_plot_function <- function(data,                    # data.frame with required columns for plotting
    ...
    if (plot_type == "series") {
        ...
        if (n_countries == 1 && n_indicators > 1 && plot_specific_args$area) {
            .print_debug("call '.plot_one_ctry_area'", verbose, debug, type = "info", "SUBPLOT")
            return(.plot_one_ctry_area)
            
        } else if (n_countries == 1 && n_indicators >= 1 && is.null(plot_specific_args$right_axis)) {
            .print_debug("call '.plot_multi_indic_one_ctry'", verbose, debug, type = "info", "SUBPLOT")
            return(.plot_multi_indic_one_ctry)
            
        } else if (n_countries %in% c(1,2) && n_indicators > 1 && !is.null(plot_specific_args$right_axis)) {
            .print_debug("call '.plot_multi_indic_one_ctry_two_axes'", verbose, debug, type = "info", "SUBPLOT")
            return(.plot_multi_indic_one_ctry_two_axes)
            
        } else if (n_countries > 1 && n_indicators == 1 && !is.null(plot_specific_args$right_axis)) {
            .print_debug("call '.plot_one_indic_multi_ctry_two_axes'", verbose, debug, type = "info", "SUBPLOT")
            return(.plot_one_indic_multi_ctry_two_axes)

        } else if (n_countries > 1 && n_indicators == 1) {
            .print_debug("call '.plot_one_indic_multi_ctry'", verbose, debug, type = "info", "SUBPLOT")
            return(.plot_one_indic_multi_ctry)
            
        # Multiple plots
        } else if (n_countries > 1 && n_indicators > 1 && plot_specific_args$area) {
            .print_debug("call '.plot_multi_area'", verbose, debug, type = "info", "SUBPLOT")
            return(.plot_multi_area)

        } else if (n_countries > 1 && n_indicators > 1 && plot_specific_args$by_indicator) {
            .print_debug("call '.plot_multi_by_indic'", verbose, debug, type = "info", "SUBPLOT")
            return(.plot_multi_by_indic)
            
        } else if (n_countries > 1 && n_indicators > 1 && !plot_specific_args$by_indicator) {
            .print_debug("call '.plot_multi_by_ctry'", verbose, debug, type = "info", "SUBPLOT")
        ...
        if (n_countries >= 1 && n_indicators == 1) {
            .print_debug("call '.plot_one_indic_bar'", verbose, debug, type = "info", "SUBPLOT")
            return(.plot_one_indic_bar)
        } else if (n_countries >= 1 && n_indicators > 1) {
            .print_debug("call '.plot_multi_indic_bar'", verbose, debug, type = "info", "SUBPLOT")
            return(.plot_multi_indic_bar)
        }
        
    } else if (plot_type == "scatter") {
        if (n_indicators == 2) {
            .print_debug("call '.plot_scatter_two_vars'", verbose, debug, type = "info", "SUBPLOT")
            return(.plot_scatter_two_vars)
        } else if (n_indicators > 2) {
            .print_debug("call '.plot_scatter_multi_vars'", verbose, debug, type = "info", "SUBPLOT")
...
.filter_args <- function(func,                    # function: target function to check arguments against
    ...
    if(length(unused_args) > 0) {
        ...
        .print_debug(msg, verbose, debug, type = "info", "ARGS")
    }
